...
.GlobalEnv$meta.vic1 <- list()     # Crea la lista "meta.vic1" en blanco. Si una lista con el mismo nombre ya existe sus contenidos serán borrados.
.GlobalEnv$meta.vic1$cat <- list() # Crea una nueva la lista en el Ambiente Global (".GlobalEnv") para guardar el resultado del loop subsecuente.
...
.GlobalEnv$meta.vic1$cat[[i]] <- data # Asigna el resultado del loop a lista "cat" en el Ambiente Global 
                                      # de cada observación "i" enlistada en  "arch_enlist"  adentro de la lista "meta.vic1".
}                                     # Cierra el loop.
})                                    # Cierra el entorno local.
...
# library(uchardet)
# meta.vic1$cat$nombredelarchivo.csv <- read.csv('nombredelarchivo.csv',
...
meta.vic1$cat           # Especifica el objeto, en este caso la lista "cat" dentro del objeto "mera.vic1".
     ...
     meta.vic1$cat),      #    1.1 El objeto cuyos nomres se van a reemplazar.
...
library(stringr) 
##Computar
ct <- meta.vic1[["cat"]]            # Se copia la lista "cat" al objeto "ct" en el entorno local, para simplificar la notación.
...
.GlobalEnv$meta.vic1[["cat"]] <- ct #Insertamos la lista con los data.frames modificados en la lista "meta.vic1$cat" dento del entorno global.
})
...
## Computar
data <- import("diccionario_de_datos_TPer_Vic1_ENVIPE_2022.csv")# Importa el contenido del archivo "diccionario_de_datos".
...
## Imprimir el resultado
.GlobalEnv$meta.vic1$dic <- data                            # Asigna el resultado "data" al entorno ".GlobalEnv" adentro de la lista "meta.vic1".
})
...
meta.vic1$nombres <- data.frame(                       # Se usa la función data.frame para crear el marco de datos.
                     ...
                     meta.vic1$dic[["nemonico"]],      # Seleccionamos el vector en este caso "nemonico" en la lista "meta.vic1" y del data.frame "dic".
                     "nombre_campo"=                   # Colocamos la cadena del nómbre que llevará el segundo vector.
                     meta.vic1$dic[["nombre_campo"]])) # Seleccionamos en este caso "nombre_campo" en la lista "meta.vic1" y del data.frame "dic".

# Si el contenido contiene celdas en blanco en el campo catálogo, y sustituirlas por NA con la función "replace"..
#meta.vic1$nombres[["catalogo"]] <- replace(meta.vic1$nombres[["catalogo"]],     # Define el nómbre del vector de búsqueda.
#                                           meta.vic1$nombres[["catalogo"]]=='', # El patrón de búsqueda son todos los valores en el vector de búsqueda está en blanco ('').
...
#meta.vic1$nombres <- drop_na(meta.vic1$nombres) # Se eliminan filas con NA con la función "drop_na".
...
meta.vic1$nombres <-meta.vic1$nombres[!duplicated(meta.vic1$nombres$catalogo), ] # Para filtran los valores duplicados por filas se usa la notación
...
meta.vic1$nombres  <- rownames_to_column(meta.vic1$nombres ,var="id") #usamos la función "rownames_to_column" para crear la variable "id".
...
require(rio)
## Computar
data <- import("") #Importa el contenido del archivo "diccionario_de_datos": "conjunto_de_datos_TPer_Vic1_ENVIPE_2022.csv"
data <- iconv.recursive (data, from="latin1") # Convierte todas las cadenas a la codificación actual "latin1".
                                              # Vic1 está en "latin1". Si se observa que no se 
                                              # respetan los acentos considere que puede estar en en "UTF-8",
                                              # por los nombres de estados y municipios.

## Imprimir el resultado
.GlobalEnv$meta.vic1$data <- data # Asigna el resultado al entorrno global (".GlobalEnv").
})

## Conviertir variables de cadena a factores
local({                                      # Abre el entorno local.
## Computar
lst_nom <- meta.vic1$factrs[["catalogo"]]    # Enlista las variables deseadas que se encuentran en la columna catálogo del objeto fctrs.
f <- meta.vic1$data                          # Para simplificar la notación copamos el objeto data al objeto "f" dentro del entorno local.
...
## Imprimir el resultado
.GlobalEnv$meta.vic1$data <-f                # Asigna el resultado al entorno global (".GlobalEnv").
})  
...
meta.vic1[["data"]][,c(10,174:177)] <- sapply(meta.vic1[["data"]][,c(10,174:177)], as.numeric)  # La función "c" envía el resultado a las variables seleccionadas.
                                                                                                ...
                                                                                                # Función para coercionar un variable a tipo "numric".
...
## Computar
df  <- meta.vic1$data                       # Para simplificar la notación copiamos el objeto "data" del entorno global al local en el objeto "df".
l   <- meta.vic1[["nombres"]][["catalogo"]] # Creamos la lista de nombres a asignarles etiquetas, presentes en el catalogo.
etq <- meta.vic1[["nombres"]]               # Copiamos el marco de datos que contiene las etiquetas al entorno local en el objeto "etq".
...
##Imprimir el resultado
.GlobalEnv$meta.vic1$data <- df             # Finalmente asigna el marco de datos resultante al objeto den el entorno global.
})
                                                                
## Selección de variables que requieren etiquetas de valor       
meta.vic1$etq.val <- meta.vic1$factrs[-(c(3:4)),]  # Crea el objeto etq.val y se descartan los objetos que ya tienen etiquetas de valor.
...
library(lookup)
## Computar
l <- meta.vic1$etq.val[["catalogo"]]  # Creamos la lista de nombres a asignarles etiquetas, presentes en el catalogo.
df <- meta.vic1$data                  # Para simplificar la notación copiamos el objeto "data" del entorno global al local en el objeto "df".
for (i in l){
f <- meta.vic1$cat[[i]]               # Copia el objeto "i" en el objeto "f". De este objeto se extraerán las etiquetas de valor.
